...
import {band, caseExpr, color, eq, Expr, literal, lt, singleCase, SingleCaseExpr} from "../map/OpenLayerExpression";
...
/**
 * Converts a color scale into an OpenLayers WebGL style object.
 *
 ...
 */
export const colorStopsToWebGLStyle = (colorStops: Array<[number, Color]>): Style => {
  const nodataCase: SingleCaseExpr<OLColor> = singleCase(
    eq(band(2), literal(0)),
    color(0, 0, 0, 0)
  );
  const colorCases: Array<SingleCaseExpr<OLColor>> =
    colorStops.map(([threshold, value]) =>
      singleCase(
        lt(band(1), literal(threshold)),
        color(value.red, value.green, value.blue, value.opacity)
      ));
  return {
    color: caseExpr(
      [nodataCase].concat(colorCases),
      color(0, 0, 0, 0) // fallback: transparent
    )
  }
}
 ...
 */
export const colorScaleToWebGLStyle = (colorScale: ColorScale): Style =>
  colorStopsToWebGLStyle(colorScale.points);

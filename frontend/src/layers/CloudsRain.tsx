...
import {
  colorScaleEl,
  colorStopsToWebGLStyle,
  Layer,
  ReactiveComponents,
  summarizerFromLocationDetails
} from "./Layer";
import {useI18n, usingMessages} from "../i18n";
import {Zone} from "../data/Model";
import {type Style} from 'ol/layer/WebGLTile';

const clearSkyStop: [number, Color] = [5, new Color(255, 255, 255, 0.00)]
const cloudCoverColorStops: Array<[number, Color]> = [
...
const cloudCoverColorScale = new ColorScale(cloudCoverColorStops);

const rainColorStops: Array<[number, Color]> = [
...
const rainColorScale = new ColorScale(rainColorStops);

/**
 * Custom WebGL style for the clouds-rain layer.
 * The backend encodes cloud cover as 0–100 and rain as value + 100.
 */
const cloudsRainWebGLStyle: Style = colorStopsToWebGLStyle(
  [clearSkyStop]
    .concat(cloudCoverColorStops)
    .concat(rainColorStops.map(([threshold, value]) => [100 + threshold, value]))
);

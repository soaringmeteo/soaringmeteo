...
import org.soaringmeteo.gfs.out.{Forecast, Raster, Store, VectorTiles, subgridTargetPath}
  ...
  def apply(
    ...
    def read(subgrid: Subgrid, gribFile: os.Path, hourOffset: Int): Future[IndexedSeq[IndexedSeq[Forecast]]] = {
      Future {
        concurrent.blocking {
          ...
          Forecast.fromGribFile(gribFile, gfsRun.initDateTime, hourOffset, subgrid)
        }
    ...
    def generateSubgridResults(subgrid: Subgrid, hourOffset: Int, forecasts: IndexedSeq[IndexedSeq[Forecast]]): Future[Unit] = {
      Future {
        ...
        Raster.writeAllPngFiles(subgrid, subgridTargetDir, hourOffset, forecasts)
        VectorTiles.writeAllVectorTiles(subgrid, subgridTargetDir, hourOffset, forecasts)
